...
byte mac[6], packetBuffer[40];
unsigned long lastEPMillis;

//settings
char lightName[LIGHT_NAME_MAX_LENGTH] = "play bar grig 2";
uint8_t scene, startup, onPin = 4, offPin = 5;
bool hwSwitch = false;
uint8_t rgb_multiplier[] = {100, 100, 100}; // light multiplier in percentage /R, G, B/

uint8_t lightsCount = 7;
uint16_t dividedLightsArray[30];

uint16_t pixelCount = 126, lightLedsCount;
uint8_t transitionLeds = 2; // pixelCount must be divisible by this value
...
void lightEngine() {
  if (lights[0].lightState) {
    if (lights[0].colors[0] != lights[0].currentColors[0] || lights[0].colors[1] != lights[0].currentColors[1] || lights[0].colors[2] != lights[0].currentColors[2]) {
      inTransition = true;
      for (uint8_t k = 0; k < 3; k++) {
        if (lights[0].colors[k] != lights[0].currentColors[k]) lights[0].currentColors[k] += lights[0].stepLevel[k];
        if ((lights[0].stepLevel[k] > 0.0 && lights[0].currentColors[k] > lights[0].colors[k]) || (lights[0].stepLevel[k] < 0.0 && lights[0].currentColors[k] < lights[0].colors[k])) lights[0].currentColors[k] = lights[0].colors[k];
      }
      strip->ClearTo(convFloat(lights[0].currentColors), 0, pixelCount - 1);
      strip->Show();
    }
  } else {
    if (lights[0].currentColors[0] != 0 || lights[0].currentColors[1] != 0 || lights[0].currentColors[2] != 0) {
      inTransition = true;
      for (uint8_t k = 0; k < 3; k++) {
        if (lights[0].currentColors[k] != 0) lights[0].currentColors[k] -= lights[0].stepLevel[k];
        if (lights[0].currentColors[k] < 0) lights[0].currentColors[k] = 0;
      }
...
void saveState() {
  ...
  JsonObject light = json.createNestedObject("0");
  light["on"] = lights[0].lightState;
  light["bri"] = lights[0].bri;
  if (lights[0].colorMode == 1) {
    light["x"] = lights[0].x;
    light["y"] = lights[0].y;
  } else if (lights[0].colorMode == 2) {
    light["ct"] = lights[0].ct;
  } else if (lights[0].colorMode == 3) {
    light["hue"] = lights[0].hue;
    light["sat"] = lights[0].sat;
  }
